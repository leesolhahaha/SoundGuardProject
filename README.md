...
#### ○ 사운드 인식 및 분류 AI
- 컴퓨터가 인식할 수 있도록 마이크에 부착된 A/D 컨버터를 사용해 소리의 파형을 디지털 신호로 변환했음.
- 환경에 따라 소음의 데시벨이 달라지므로, 소리의 인식 반경은 신호 대 잡음비(SNR)에 따라 조정했음.
- 잡음에 의해 오염된 음성 신호는 각 프레임으로 분리되어 해밍창에 통과시킨 후 고속 푸리에 변환(FFT)을 통해 주파수 이미지를 Spectrogram 파형으로 변환했음.
- 이 데이터를 특징 벡터(feature)화 해주는 MFCC 알고리즘을 적용하고, 파이썬의 `librosa` 라이브러리를 사용하여 이를 추출했음.
- 소리 데이터를 발견하고 노이즈를 제거하는 전처리 과정을 거쳐, 최종적으로 학습 피처를 추출했음.
- 추출된 피처를 바탕으로 CNN 신경망을 사용하여 AI 모델을 학습시켰음.
- 인식된 사운드를 통해 인공지능이 단순 생활 소음인지 비명소리인지를 판단하고, 상황에 맞게 분류했음.
    <br>
...
- **데이터셋**:
  - 학습에 활용한 데이터셋은 유튜브 및 무료 사운드 제공 사이트에서 정제하여 구성한 파일과 AI-Hub에서 다운로드한 파일을 사용했음.
  - 각 WAV 파일은 3초 단위로 고정하고 라벨링 파일을 생성하여 인공지능 학습에 활용했음.
  - 학습 데이터의 양을 증가시키기 위해 3,000개의 데이터를 12,000개로 증강하여 사용했음.

#### ○ 데이터 통계 대시보드
- 수집한 데이터를 활용하여 이상 음원 다발 발생 장소와 특정 발생 시간대, 신고 접수 데이터 등을 기반으로 DB를 구축했음.
- 데이터 마이닝의 연관분석 알고리즘을 활용하여, 이상 음원 다발 발생 장소 및 특정 발생 시간대와 관련된 통계를 추출했음.
- 이를 바탕으로 범죄 발생 예측 테이블을 작성하고, 대시보드를 통해 정보를 제공했음.

#### ○ 자동 신고
- 분석 완료된 데이터에 기반하여 위급상황으로 판단되면, 인공지능이 해당 소리가 수집된 위치 반경 내의 관할서 혹은 경찰서의 관제 시스템에 자동으로 신고했음.

### 2) H/W

#### ○ 라즈베리파이
- 비명 인식기 스피커에 내장된 IoT 기기의 본체로, `라즈베리파이` 4B 모델을 사용했음.
- 듀얼 마이크 확장 보드인 `ReSpeaker 2-Mics Pi HAT`을 사용하여 소리를 인식하고 WAV 파일 형식으로 저장했음.

#### ○ GPS 모듈
- 비명이 발생한 위치 정보를 파악하기 위해 `GPS 모듈`을 설치했음.
- 비명 발생 위치와 주변 관할서 위치를 파악하는 데 사용되었으며, 사건의 정확한 위치를 추적하여 신고 시 중요한 정보를 제공했음.
...
| 구분       | 상세내용                |
|------------|-------------------------|
| **개발환경** | OS: Linux Ubuntu 22.04 LTS |
| **개발환경(IDE)** | Google Colab            |
| **개발도구** | 노트북                   |
| **개발언어** | Python, JAVA, HTML, CSS, JavaScript, React |
| **기타사항** | -                       |
...
| 구분       | 상세내용                |
|------------|-------------------------|
| **구성장비** | 디바이스: 라즈베리파이4, 마이크(ReSpeaker2-Mics Pi HAT) |
| **센서**    | -                       |
| **통신**    | GPS 모듈, Wifi          |
| **언어**    | Python                  |
| **기타사항** | -                       |
...
![스피커 사진](images/speaker_image.png)  ![웹페이지 사진](images/webpage_image.png)
...
- **게재(발표)자명**: 오윤선
- **논문(포스터)명**: 여성안심 귀갓길 비명 인식 인공지능 시스템
- **게재(발표)처**: 한국정보처리학회
- **게재(발표)일자**: 2023. 10. 12.

![논문 사진 1](images/paper1.png)  ![논문 사진 2](images/paper2.png)

### 공모전 수상

- **4차 산업혁명 기반 2023년 창의융합형 인재양성 공유협업 페스티벌**
  - **캡스톤디자인산출물 부문 우수상**
  - **사회문제해결아이디어 부문 장려상**
...
- 유광현, "딥러닝 기반의 비명소리 인식 및 구현", 공학석사학위논문 전남대학교 대학원, pp.7, 2018
- 이소연 외 3명, "음향 데이터 기반 지능형 상황 인식 시스템", 대한전자공학회 하계학술대회 논문집, pp.2731, 2022
- 이소연 외 2명, “음향 데이터를 활용한 딥러닝 기반 긴급차량 우선 신호 시스템”, Journal of Platform Technology, 제 9권, 제 3호, pp.47-49, 2021
